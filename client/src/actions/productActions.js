...
export const listProductsByCategory = (category, pageNumber = '') => async (dispatch) => {
    try{
        ...
        const { data } = await axios.get(`/products/category?keyword=${category}&pageNumber=${pageNumber}`);
        console.log("data from list by category is: ", data);
        ...
        dispatch({type: PRODUCT_BY_CATEGORY_FAIL, payload: error.response && error.response.data.message ? error.response.data.message : error.message })
    };
};
